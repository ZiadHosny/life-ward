...
import TraderModal from "../../Components/Traders/TraderModal";
...
function TradersPage() {
  ...
  useEffect(() => {
    const id = setTimeout(() => {
      if (inputSearch) {
        ...
        removeArrayFromSearch(["keyword[name]"]);
      }
    }, 500);
  ...
  const breadcrumbs = [
    ...
    <Typography key="3" color="text.primary" sx={{ color: "#00e3d2" }}>
      {language === "en" ? "Traders" : "التجار"}
    </Typography>,
  ];

  return (
    <>
      <TraderModal open={open} setOpen={setOpen} />
        ...
        <Box sx={{ py: "40px", px: "20px" }}>
          ...
          >
            ...
            {allowed({ page: "traders", role }) ? (
              ...
              >
                <Typography>
                  {language === "en" ? "Add Trader" : "اضافة تاجر"}
                </Typography>
              </Button>
            ) : (
              <></>
