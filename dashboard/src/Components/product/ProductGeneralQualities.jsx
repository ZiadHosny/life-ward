...
const ProductGeneralQualities = ({
  ...
  useEffect(() => {
    if (productQualities?.length > 0) {
      const filterSelectedGeneralQualities = productQualities?.flatMap((el) => {
        const DindMatch_2 = el?.values?.map(({ key_en, key_ar }) => {
          const findedMatchKey = dataQualities?.data?.find((item) => {
            return key_en === item?.key_en;
          });
          if (findedMatchKey) {
            return findedMatchKey?._id;
          }
          return;
        });
      ...
      const uniqueIds = Array.from(
        new Set(filterSelectedGeneralQualities)
      )

      if (uniqueIds && uniqueIds[0]) {
        produuctSetFieldValue("generalQualities", uniqueIds);
      }
    } else {
  ...
  useEffect(() => {
    if (editedProduct) {
      const filterSelectedGeneralQualities =
      ...
      setSortedQualities(sortedFiltereds);
    }
  }, [editedProduct]);
  return (
    <Box>
