...
const ProductQualities = ({
  ...
  const handleAddValueToQuality = () => {
    if (editedValueItem) {
      const updatedValue = qualityFormik.values.values.map((el) =>
        el.value_en === editedValueItem.value_en
          ? { ...qualityValue, price: Number(qualityValue.price) }
          : el
      );
      qualityFormik.setFieldValue("values", updatedValue);
      setEditedValueItem();
      setQualityValue({
        value_en: "",
        value_ar: "",
      ...
      const { value_en, value_ar, color } = qualityValue;
      if (qualityFormik.values.key_en === "colors" && color === "") {
        ...
        const qualitiesValues = productQualities.reduce(
          (acc, item) => acc.concat(item.values),
          []
        );
        const findExisted = qualitiesValues.find(
          (val) => val.value_en === value_en || val.value_ar === value_ar
        );
        if (findExisted) {
          toast.error(
            language === "en"
              ? `There are similar names for values used in product qualities`
              : `هناك أسماء متشابهة لقيم مستخدمة في معايير المنتج`
          );
        } else {
          setValues({
            ...values,
  ...
  useEffect(() => {
    if (colorSwitch) {
      ...
      setFieldValue("values", editedQual ? editedQual.values : []);
      setQualityValue({
        value_en: "",
        value_ar: "",
        color: "",
      ...
      setFieldValue("key_en", editedQual?.key_en || "");
      setFieldValue("key_ar", editedQual?.key_ar || "");
      setFieldValue("values", editedQual?.values || []);
      setQualityValue({
        value_en: "",
        value_ar: "",
        price: 0,
  ...
  const [editedValueItem, setEditedValueItem] = useState();
  const handleSelectValueToEdit = (item) => {
    setEditedValueItem(item);
    if (item?.color) {
      setQualityValue({
      ...
      >
        ...
        >
          <Stack direction={"row"} justifyContent={"space-between"} width={1}>
            ...
            >
              {language === "en" ? "Qualities" : "المعايير"}
            </Typography>
            <Button
        ...
        <AccordionDetails>
          ...
          >
            ...
            <Typography
              ...
              sx={{ cursor: "pointer" }}
            >
              {language === "en" ? "Add color quality" : "أضف لون معيار"}
            </Typography>
          </Stack>
          ...
          <Box>
            <InputText
              ...
              error={errors.key_en}
              value={values.key_en}
              touched={touched.key_en && !colorSwitch}
            ...
            <InputText
              ...
              error={errors.key_ar}
              value={values.key_ar}
              touched={touched.key_ar && !colorSwitch}
              handleChange={handleChange}
              handleBlur={handleBlur}
              isDisabled={colorSwitch}
            />
                ...
                >
                  {values.values.map((item) => (
                    <Box
                      ...
                      wordBreak={`break-all`}
                    >
                      {item.value_en !== editedValueItem?.value_en ? (
                        <EditIcon
                          sx={{
                            position: "absolute",
                            top: 0,
                            left: language === "ar" ? "25px" : undefined,
                            right: language === "en" ? "25px" : undefined,
                            cursor: "pointer",
                            color: colors.main,
                            fontSize: "20px",
                            transform:
                          ...
                          onClick={() => handleSelectValueToEdit(item)}
                        />
                      ) : undefined}
      ...
      {productQualities?.length > 0 ? (
        <Box>
          {productQualities.map((productQuality) => (
            ...
            >
              <Typography py = {'5px'} >{productQuality[`key_${language}`]}</Typography>
              <CloseIcon
