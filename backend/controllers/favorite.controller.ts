  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(searchData.length === 0){
      return next(
        new ApiError(
          {
            en: "not found items in your favourite list",
            ar: "لا يوجد عناصر في قائمة المفضلة الخاصة بك",
          },
          StatusCodes.NOT_FOUND
        )
      );
    ...
    res.status(200).json({
      status: Status.SUCCESS,
      data: result,
      success_en: "data retrieved successfully ",
      success_ar: "تم استرجاع البيانات بنجاح",
    });
  }
);
...
export const toggleProductFromFavouritesList = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (isExist) {
      ...
      megEn = "Deleted from favorites";
      megAr = "تم الحذف من المفضلة";
    } else {
      await User.findByIdAndUpdate(
        (req.user! as any)._id,
        {
      ...
      megEn = "Added to favorites";
      megAr = "تم الاضافة الي المفضلة";
    }
